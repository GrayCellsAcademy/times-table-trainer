...
function createUser(name, role, classId=null) {
  const id = Date.now().toString(36) + Math.random().toString(36).slice(2);
  return {
    id, name, role, classId,
    progress: { currentTable: MIN_TABLE, masteredTables: [], stage:"s1", certificates:[] }
  };
}
function createClass(name, password, teacherId) {
  const id = "cls_" + Date.now().toString(36);
  return { id, name, password, teacherId, studentIds:[] };
}
...
function WrongPanel({wrong,onDismiss}) {
  return (
    <div style={{background:"rgba(239,68,68,0.13)",border:"2px solid rgba(239,68,68,0.5)",borderRadius:20,padding:"16px 20px",animation:"bounceIn 0.3s ease"}}>
      <div style={{fontSize:"clamp(16px,3.5vw,22px)",color:"#fca5a5",fontFamily:"'Fredoka One',cursive",marginBottom:7}}>Not quite! 😅</div>
...
function TeacherDashboard({user, onBack}) {
  const db = loadDB();
  const cls = user.classId ? db.classes[user.classId] : null;
  const [showCreate, setShowCreate] = useState(!cls);
  ...
  const createCls = () => {
    ...
    mutateDB(db => {
      const cls = createClass(className.trim(), classPass.trim(), user.id);
      db.classes[cls.id] = cls;
      db.users[user.id].classId = cls.id;
    });
    setShowCreate(false);
    window.location.reload();
  };

  const students = cls ? cls.studentIds.map(id=>db.users[id]).filter(Boolean) : [];
  ...
  if (showCreate) return (
    <div style={{...card,textAlign:"center",animation:"bounceIn 0.4s ease"}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"center"}}>
        ...
        <button onClick={createCls} style={{fontFamily:"'Fredoka One',cursive",padding:"11px 28px",borderRadius:99,border:"none",background:"#fbbf24",color:"#fff",cursor:"pointer",fontSize:16,boxShadow:"0 3px 14px #fbbf2488"}}>Create Class 🏫</button>
      </div>
    </div>
  );
...
export default function TimesTableApp() {
  ...
  useEffect(()=>{
    const db=loadDB();
    const lastId=localStorage.getItem("ttt_lastUser");
    if (lastId&&db.users[lastId]) {
      const u=db.users[lastId];
      setCurrentUser(u);
      loadUserProgress(u);
      setAppScreen(u.role==="teacher"?"teacher_home":"student_home");
    } else {
      setAppScreen("signup");
    }
  },[]);

  const loadUserProgress=(u)=>{
    const p=u.progress;
    setCurrentTable(p.currentTable||MIN_TABLE);
    setMasteredTables(p.masteredTables||[]);
    setGameScreen("home");
  };

  // ── Persist progress ─────────────────────────────────────────
  const persistProgress=(table, mastered, stage)=>{
    if (!currentUser) return;
    mutateDB(db=>{
      if (db.users[currentUser.id]) {
        db.users[currentUser.id].progress.currentTable=table;
        db.users[currentUser.id].progress.masteredTables=mastered;
        db.users[currentUser.id].progress.stage=stage;
      }
    });
  };
  ...
  // ── Signup ───────────────────────────────────────────────────
  const doSignup=()=>{
    if (!signupName.trim()||!signupRole){ setSignupErr("Please enter a name and choose a role."); return; }
    const user=createUser(signupName.trim(), signupRole);
    mutateDB(db=>{ db.users[user.id]=user; });
    localStorage.setItem("ttt_lastUser",user.id);
    setCurrentUser(user);
    if (signupRole==="teacher") { setAppScreen("teacher_home"); }
    else { setAppScreen("join_class"); }
  };

  const doJoinClass=()=>{
    const db=loadDB();
    const cls=Object.values(db.classes).find(c=>c.name.toLowerCase()===joinClassName.trim().toLowerCase()&&c.password===joinClassPass.trim());
    if (!cls){ setSignupErr("Class not found or wrong password."); return; }
    mutateDB(db=>{
      db.users[currentUser.id].classId=cls.id;
      if (!cls.studentIds.includes(currentUser.id)) db.classes[cls.id].studentIds.push(currentUser.id);
    });
    setCurrentUser(u=>({...u,classId:cls.id}));
    loadUserProgress(currentUser);
    setAppScreen("student_home");
  };

  const skipJoinClass=()=>{ setAppScreen("student_home"); };
  ...
  const dismissS2Wrong=()=>{setS2Wrong(null);setS2Input("");setS2Idx(p=>(p+1)>=s2Questions.length?0:p+1);setTimeout(()=>s2Ref.current?.focus(),50);};
      ...
      if(questions[idx].streak>=questions[idx].streakNeeded){
        ...
        if(questions.length===0){
          ...
          if(newMastered.length===11){
            ...
            mutateDB(db=>{
              if(db.users[currentUser.id]){
                db.users[currentUser.id].progress.masteredTables=newMastered;
                db.users[currentUser.id].progress.certificates.push(cert);
              }
            });
            ...
            setCelebration({
              ...
              onContinue:()=>{
                setCelebration(null);setSuperCelebration(false);
                const db2=loadDB();
                const certs=db2.users[currentUser.id]?.progress?.certificates||[];
                setViewCert(certs[certs.length-1]);
  ...
  const dismissS3Wrong=()=>{setS3Wrong(null);setS3Input("");setS3Idx(p=>p%s3Questions.length);setTimeout(()=>s3Ref.current?.focus(),50);};
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#1e1b4b 0%,#312e81 40%,#4c1d95 100%)",fontFamily:"'Nunito',sans-serif",padding:"clamp(12px,2vw,24px)",boxSizing:"border-box",position:"relative",overflow:"hidden"}}>
      ...
      {/* ══ SIGNUP ══ */}
      {appScreen==="signup"&&(
        <div style={{...card,textAlign:"center",animation:"bounceIn 0.5s ease"}}>
          ...
          <p style={{color:"rgba(255,255,255,0.7)",fontSize:"clamp(12px,2.2vw,15px)",marginBottom:18}}>Create your account to get started!</p>
          {signupErr&&<div style={{color:"#f87171",fontSize:13,marginBottom:10}}>{signupErr}</div>}
          <input value={signupName} onChange={e=>setSignupName(e.target.value)} placeholder="Your name" style={{width:"100%",boxSizing:"border-box",padding:"12px 16px",borderRadius:14,border:"2px solid rgba(255,255,255,0.28)",background:"rgba(255,255,255,0.13)",color:"#fff",fontSize:16,fontFamily:"'Nunito',sans-serif",marginBottom:16,outline:"none"}}/>
          <div style={{marginBottom:18}}>
            <div style={{fontSize:"clamp(12px,2.2vw,15px)",color:"rgba(255,255,255,0.7)",marginBottom:10}}>I am a…</div>
            <div style={{display:"flex",gap:12,justifyContent:"center"}}>
              {[["student","🧒 Student"],["teacher","👩‍🏫 Teacher"]].map(([role,label])=>(
                <button key={role} onClick={()=>setSignupRole(role)} style={{...btn(signupRole===role?"#f472b6":"rgba(255,255,255,0.13)",{boxShadow:signupRole===role?undefined:"none",border:signupRole===role?"none":"2px solid rgba(255,255,255,0.22)",fontSize:"clamp(15px,3vw,19px)",padding:"13px 24px"})}}>
                  {label}
                </button>
              ))}
            </div>
          </div>
          <button style={btn("#fbbf24",{fontSize:"clamp(16px,3.2vw,22px)",padding:"13px 36px",width:"100%"})} onClick={doSignup}>Create Account →</button>
          <div style={{marginTop:14,fontSize:"clamp(11px,2vw,13px)",color:"rgba(255,255,255,0.4)"}}>Already have an account on this device?{" "}
            <span style={{color:"#818cf8",cursor:"pointer",textDecoration:"underline"}} onClick={()=>{
              const db=loadDB(); const names=Object.values(db.users).map(u=>u.name).join(", ");
              setSignupErr(names?`Accounts on this device: ${names}. Enter your name above and tap Create Account.`:"No accounts found.");
            }}>Tap here</span>
          </div>
        </div>
      )}
      ...
      {appScreen==="join_class"&&(
        <div style={{...card,textAlign:"center",animation:"bounceIn 0.4s ease"}}>
          ...
          {signupErr&&<div style={{color:"#f87171",fontSize:13,marginBottom:10}}>{signupErr}</div>}
          <input value={joinClassName} onChange={e=>setJoinClassName(e.target.value)} placeholder="Class name" style={{width:"100%",boxSizing:"border-box",padding:"11px 14px",borderRadius:13,border:"2px solid rgba(255,255,255,0.26)",background:"rgba(255,255,255,0.12)",color:"#fff",fontSize:15,fontFamily:"'Nunito',sans-serif",marginBottom:10,outline:"none"}}/>
          <input value={joinClassPass} onChange={e=>setJoinClassPass(e.target.value)} placeholder="Class password" style={{width:"100%",boxSizing:"border-box",padding:"11px 14px",borderRadius:13,border:"2px solid rgba(255,255,255,0.26)",background:"rgba(255,255,255,0.12)",color:"#fff",fontSize:15,fontFamily:"'Nunito',sans-serif",marginBottom:16,outline:"none"}}/>
          <div style={{display:"flex",gap:10,justifyContent:"center"}}>
            <button onClick={skipJoinClass} style={btn("rgba(255,255,255,0.11)",{boxShadow:"none",border:"1px solid rgba(255,255,255,0.2)",fontSize:14})}>Skip for now</button>
      ...
      {appScreen==="teacher_home"&&(
        <div style={{...card,textAlign:"center",animation:"bounceIn 0.4s ease"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            <button style={btn("rgba(255,255,255,0.12)",{fontSize:"clamp(13px,2.5vw,16px)",padding:"10px",boxShadow:"none",border:"1px solid rgba(255,255,255,0.2)",marginTop:6})}
              onClick={()=>{localStorage.removeItem("ttt_lastUser");setAppScreen("signup");setCurrentUser(null);}}>
              Switch Account
            </button>
          </div>
        </div>
      ...
      {appScreen==="student_home"&&(
        <div style={{...card,animation:"bounceIn 0.4s ease"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,marginBottom:20}}>
            ...
            <button style={btn("rgba(255,255,255,0.1)",{fontSize:"clamp(11px,1.6vw,13px)",padding:"8px 16px",boxShadow:"none",border:"1px solid rgba(255,255,255,0.18)"})}
              onClick={()=>{localStorage.removeItem("ttt_lastUser");setAppScreen("signup");setCurrentUser(null);}}>
              Switch Account
            </button>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))",gap:16}}>
            ...
            <div style={{background:"rgba(255,255,255,0.07)",borderRadius:18,padding:"clamp(14px,2.5vw,28px)",display:"flex",flexDirection:"column",gap:12}}>
              ...
              {(()=>{
                const db=loadDB();
                const certs=db.users[currentUser?.id]?.progress?.certificates||[];
                if(certs.length===0) return null;
                return (
                  <button style={btn("#fbbf24",{fontSize:"clamp(14px,2.2vw,18px)",padding:"14px",width:"100%",textAlign:"left"})} onClick={()=>setViewCert(certs[certs.length-1])}>
                    🏆 View My Certificate{certs.length>1?"s":""}
                  </button>
                );
              })()}
            </div>
